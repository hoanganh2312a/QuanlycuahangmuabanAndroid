...
public class DBSanPham extends SQLiteOpenHelper {
    ...
    public void SuaDL(SanPham sp){
        String sql="Update tbSanPham set tensp=?,giasp=?,loaisp=?,hinhsp=?,soluongsp=?where masp=?";
        SQLiteDatabase sqLiteDatabase=getWritableDatabase();
        sqLiteDatabase.execSQL(sql,new String[]{sp.getTenSP(), sp.getGiaSP(), sp.getLoaiSP(), sp.getHinhSP(), sp.getSoluongSP(),sp.getMaSP()});
    }
}
